...
excel_file <- Sys.glob('Популяция_Хонгор_2_года_Москва_и_Краснодар.xlsx')

main_table <- readxl::excel_sheets(excel_file) %>% 
  purrr::map_dfr(
    \(x){
...
index <-  'п_п'
factors <- c('раст_2017', 'семья', 'место', 'год', 'ddw1', 'vrna1', 'grf', 'grf9_6a_12del')
...
Рассмотрим модели типа "\~ (grf9_6a_12del + ddw1 + vrna1 + место + год)\*\*3" для всех наблюдаемых параметров для grf9_6a_12del на уровнях 0 и 2.
...
ggplot(main_table, aes(x=!!sym(x))) + 
  ...
  facet_grid(grf9_6a_12del +  ddw1 + vrna1  ~ место + год, labeller = label_parsed, 
             drop = TRUE, scales = 'free') -> ggplot_local

main_table %>% 
  select(-where((is.numeric)), !!x) %>% 
  group_by(grf9_6a_12del, ddw1, vrna1, место, год) %>% 
...
anova <- aov(as.formula(glue::glue("{x} ~ (grf9_6a_12del + ddw1 + vrna1 + место + год)**3")), data = main_table)
